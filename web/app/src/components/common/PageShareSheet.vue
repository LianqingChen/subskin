...
function shareToWeibo() {
  const weiboUrl = `https://service.weibo.com/share/share.php?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(props.title)}`
...
function shareToWechat() {
  if (isWechat && isReady.value) {
    setShareData({
      title: props.title,
      desc: props.description || props.title,
      link: shareUrl,
      ...
      <div v-if="visible" class="fixed inset-0 z-50 flex items-end md:items-center justify-center" @click.self="closeSheet">
        ...
        <div class="relative bg-white w-full md:max-w-md md:rounded-2xl rounded-t-2xl max-h-[80vh] overflow-y-auto z-10">
          ...
          <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
            ...
            <button type="button" class="text-gray-400 hover:text-gray-600 text-2xl leading-none" @click="closeSheet">&times;</button>
          </div>

          <!-- Share Buttons Grid -->
          <div class="grid grid-cols-4 gap-4 p-5">
            <!-- Poster -->
            <button type="button" class="flex flex-col items-center gap-2 group" @click="generatePoster">
            ...
            <!-- Copy Link -->
            <button type="button" class="flex flex-col items-center gap-2 group" @click="copyLink">
            ...
            <!-- WeChat -->
            <button type="button" class="flex flex-col items-center gap-2 group" @click="shareToWechat">
            ...
            <!-- Moments -->
            <button type="button" class="flex flex-col items-center gap-2 group" @click="shareToMoments">
            ...
            <!-- Weibo -->
            <button type="button" class="flex flex-col items-center gap-2 group" @click="shareToWeibo">
            ...
            <!-- Douyin -->
            <button type="button" class="flex flex-col items-center gap-2 group opacity-60" @click="shareToDouyin">
            ...
            <!-- Xiaohongshu -->
            <button type="button" class="flex flex-col items-center gap-2 group opacity-60" @click="shareToXiaohongshu">
          ...
          <div class="px-5 pb-6 md:hidden">
            <button type="button" class="w-full py-3 text-center text-sm text-gray-500  bg-gray-50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors" @click="closeSheet">取消</button>
          </div>
        </div>
      </div>
    </Transition>
